...
export default function DashboardLayout({ children }: { children:React.ReactNode }) {
  ...
  /* Schedule badge — live count */
  const [scheduleBadge, setScheduleBadge] = useState(0);

  /* Clients for sidebar switcher */
  const [clients,      setClients]      = useState<{id:string;name:string}[]>([]);
  const [activeClient, setActiveClient] = useState<string>("");

  /* Poll live data: badge + notifications + clients */
  const pollData = useCallback(() => {
    fetch("/api/posts").then(r=>r.json()).then(res => {
      ...
      setScheduleBadge(pending);

      // Live notifications from real posts
      ...
      const failed = posts.filter(p => p.status === "failed").slice(0, 2);
      const pendingApproval = posts.filter(p => p.status === "pending").slice(0, 1);

      published.forEach(p => newNotifs.push({
      ...
      failed.forEach(p => newNotifs.push({
        msg:  `Failed: ${p.content.substring(0,50)}${p.content.length>50?"…":""} — ${p.errorMsg||"unknown error"}`,
        time: "Check schedule",
        type: "warn",
      }));
      pendingApproval.forEach(p => newNotifs.push({
        msg:  `Awaiting client approval: ${p.content.substring(0,50)}…`,
        time: "Pending",
        type: "info",
      }));
      ...
      if (newNotifs.length === 0) {
        newNotifs.push({
          msg:  "No activity yet — compose your first post",
          time: "",
          type: "info",
        });
      }
    ...
    fetch("/api/clients").then(r=>r.json()).then(res => {
      if (res.ok && Array.isArray(res.data) && res.data.length > 0) {
        ...
        setClients(list);
        setActiveClient(prev => prev || list[0].id);
      }
    }).catch(()=>{});
  }, []);
  ...
  useEffect(() => {
    if (status === "authenticated") {
      pollData();
      const id = setInterval(pollData, 30_000);
          ...
          {NAV.map(g => (
            <div key={g.group}>
              ...
              {g.items.map(item => {
                ...
                const badgeCount = item.badge ? scheduleBadge : 0;
                return (
      ...
      <main className={`main-content${collapsed?" sidebar-collapsed":""}`}>
        ...
        <header className="topbar">
          ...
          <div className="topbar-title">
            {pageTitle}
            <span className="sub"> / {pathname==="/dashboard"?"Overview":pageTitle}</span>
          </div>

          <div className="topbar-actions">
            <div className="theme-toggle">
              <button className={`theme-opt${theme==="dark"?" on":""}`}
                onClick={()=>switchTheme("dark")} aria-label="Dark"><MoonIco/></button>
              <button className={`theme-opt${theme==="light"?" on":""}`}
                onClick={()=>switchTheme("light")} aria-label="Light"><SunIco/></button>
            </div>

            <button ref={notifBtnRef} className="icon-btn"
              onClick={()=>setNotifOpen(o=>!o)} aria-label="Notifications">
              <BellIco/>
              {notifs.some(n=>n.type!=="info") && <div className="notif-badge"/>}
            </button>
          ...
          <div ref={notifRef} className="notif-panel">
            <div className="notif-head">
              ...
              <button className="btn btn-ghost btn-sm"
                onClick={()=>{ pollData(); setNotifOpen(false); }}>
                Refresh
              </button>
            </div>
            {notifs.map((n,i)=>(
              <div key={i} className="notif-item"
                ...
                <div className="notif-dot-icon" style={{
                  background: n.type==="success"?"var(--success-dim)":n.type==="warn"?"var(--gold-dim)":"var(--blue-dim)",
                  color:      n.type==="success"?"var(--success)":n.type==="warn"?"var(--gold)":"var(--blue)",
                }}>
                  {n.type==="success"?<CheckIco/>:n.type==="warn"?<WarnIco/>:<InfoIco/>}
                </div>
