...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  /* * FIX APPLIED HERE: 
   * loadBadge is now wrapped in useCallback so it is only created once.
   * The polling interval is changed to 120_000 (2 minutes) to prevent aggressive re-rendering.
   */
  ...
  useEffect(() => {
    loadBadge();
    const id = setInterval(loadBadge, 120_000); // 2 minutes — background sync only
  ...
  /* * FIX APPLIED HERE:
   * Added roleChecked useRef to ensure the role fetch only happens exactly once per session.
   * This breaks the infinite Layout refresh loop.
   */
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && !roleChecked.current) {
      roleChecked.current = true; // Lock it down so it never fires again this session
