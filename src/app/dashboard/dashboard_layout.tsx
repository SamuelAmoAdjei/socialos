...
const NOTIFS = [
  { msg: "Post published to LinkedIn for Acme Corp", time: "2 minutes ago", type: "success" },
  { msg: "2 draft posts are awaiting your approval",  time: "1 hour ago",   type: "info" },
  { msg: "Buffer connection requires re-authorisation", time: "3 hours ago", type: "warn" },
];
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("socialos-theme") as "dark" | "light" | null;
  ...
  function switchTheme(t: "dark" | "light") {
    ...
    localStorage.setItem("socialos-theme", t);
  }
  ...
  useEffect(() => {
    const saved = localStorage.getItem("socialos-sidebar");
  ...
  function toggleCollapse() {
    ...
    setCollapsed(next);
    localStorage.setItem("socialos-sidebar", next ? "collapsed" : "expanded");
  }
  ...
  const pageSubtitle = pathname === "/dashboard" ? "Overview" : pageTitle;
      ...
      <main className={`main-content${collapsed ? " sidebar-collapsed" : ""}`}>
        ...
        <header className="topbar">
          ...
          <div className="topbar-title">
            {pageTitle} <span className="sub">/ {pageSubtitle}</span>
          </div>

          <div className="topbar-actions">
            {/* Theme */}
            <div className="theme-toggle">
              <button className={`theme-opt${theme === "dark" ? " on" : ""}`} onClick={() => switchTheme("dark")} aria-label="Dark mode">
                <MoonIco />
              </button>
              <button className={`theme-opt${theme === "light" ? " on" : ""}`} onClick={() => switchTheme("light")} aria-label="Light mode">
                <SunIco />
              </button>
            </div>
            ...
            {/* Sign out */}
            <button className="signout-btn" onClick={() => signOut({ callbackUrl: "/auth/signin" })}>
              Sign out
            </button>
          </div>
        </header>
...
function CheckIco() {
  return <svg width="15" height="15" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
